-- ######################################################################
-- # FASE 1: CREACIÓN DE ESTRUCTURA (DDL)
-- ######################################################################

-- 1. ELIMINACIÓN SEGURA DE OBJETOS EXISTENTES (Para re-ejecución)
DROP TABLE IF EXISTS Cliente_telefono CASCADE;
DROP TABLE IF EXISTS Pago_transaccion CASCADE;
DROP TABLE IF EXISTS Asignacion_cocina CASCADE;
DROP TABLE IF EXISTS Entrega_domicilio CASCADE;
DROP TABLE IF EXISTS Atencion_Pedido CASCADE;
...
-- 2. CREACIÓN DE TABLAS PADRE (Incluye Subtipos, Mesas, Platillos)
CREATE TABLE Mesero (
